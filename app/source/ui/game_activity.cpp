...
///
/// Ширина у трейлера и снимка одна: при разных размерах полоса читается как
/// «две кнопки и картинки» вместо одного ряда превью.
...
/// стороны — 1220 точек; пять плиток занимают 1190. Трейлеры и снимки делят
/// общий предел: если считать их порознь, полоса вылезает за край экрана, а
/// доскроллить до последнего снимка нечем — прокрутка на карточке вертикальная.
...
///
/// Не подпись цветом text_disabled: тем же цветом в теме покрашены недоступные
/// элементы, и «Есть русский» читалось бы как отключённая кнопка, а не как
/// факт об игре.
...
{
    // Детали читаем из каталога в рабочем потоке, и только потом открываем экран.
...
{
    // Те же кнопки, что в каталоге: X кладёт в списки, Y прячет. Одна кнопка не
    // должна значить разное на разных экранах.
...
{
    // Жанры в каталоге хранятся по-русски — это значения, а не подписи. При
    // английском интерфейсе их переводит genreLabel().
    std::string joined;
...
void GameActivity::fillStats()
{
    ...
    // Плитки «Статус» здесь нет намеренно. Полезен только положительный ответ,
    // а он приходится на одну игру из полусотни — остальные сорок девять
    // отдавали бы четверть строки под слово «Не установлена». Установленные
    // помечены чипом среди прочих меток, ниже.
}
...
void GameActivity::fillScreenshots(std::vector<std::string> urls, int slots)
{
    ...
    // из сети и оседают в кэше на SD. В полосе — сколько осталось мест после
    // трейлеров: остальные всё равно не видны без прокрутки, а трафик тратят.
    // В галерею отдаём все: листать там есть куда, а трафик тратится только на
    // открытый снимок.
    ...
    for (size_t i = 0; i < urls.size(); i++)
    {
        ...
        // Снимок фокусируется и открывается: иначе фокус достаётся только
        // кнопке трейлера, и разглядеть скриншоты нечем.
        ...
        shot->onDone = [this](bool ok) {
            ...
            // не загрузилось — иначе она висит на экране вечно
            if (shotsFailed == 0)
                shotsHint->setText("");
            else
...
{
    // Ролики есть у 61% игр (2139 из 3489); у остальных ставим disabled-кнопку
    // с подписью, а не прячем её: так понятнее, что раздел вообще существует.
    // У большинства игр ролик один, но у 295 их несколько — показываем кнопку
    // на каждый, но не больше двух: третий трейлер вытеснил бы из полосы все
    // снимки, а показать игру они умеют лучше.
    if (videos.size() > 2)
        videos.resize(2);
    ...
    // открытая в первые секунды после запуска, получила бы «нет сети» до самого
    // закрытия, хотя сеть успевает подняться. Проверяем в момент нажатия, где
    // можно и подождать.
...
void GameActivity::openTrailer()
{
    ...
    // Подпись возвращаем в любом случае, иначе на ветке отказа она остаётся
