...
void MainTabs::select(size_t index)
{
    ...
    //
    // Через brls::sync, а не сразу: страница только что стала VISIBLE и ещё
    // не переложена — она была GONE, то есть нулевого размера. giveFocus тут
    // же попытался бы прокрутить сетку к фокусу по этим нулевым координатам,
    // и на следующем кадре, когда раскладка досчитывается по-настоящему,
    // прокрутка дёргалась на разницу — сетка «падала» сверху на треть плитки.
    // sync выполняется в начале следующего кадра, уже после того как эта
    // раскладка отработает.
    if (switching)
        brls::sync([this, index]() { brls::Application::giveFocus(pages[index]); });
}
