...
///
/// Показ начинается до того, как ролик докачан: качать мегабайты по Wi-Fi
/// целиком — десятки секунд ожидания перед первым кадром. Оба условия для
...
class HttpStream
{
  public:
    ...
    /// Сеть сдалась окончательно: попытки переподключения исчерпаны или
    /// сервер не умеет докачку. Дальше readPacket отдаёт ошибку, а не конец
    /// файла — иначе плеер принял бы обрыв за финал ролика и ушёл бы в цикл
    /// «перемотать в начало — снова обрыв».
    bool failed() const { return connectionFailed.load(); }
