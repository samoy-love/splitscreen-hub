...
/// консоль оставит обрезанный JSON и библиотека потеряется целиком. Прежняя
/// версия остаётся рядом как .bak: если основной файл всё же не читается,
/// load() берёт её, а испорченный отодвигает в .corrupt, не затирая.
class Library
{
  public:
