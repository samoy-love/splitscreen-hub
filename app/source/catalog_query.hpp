...
/// Отбор и порядок игр — без borealis и без файлов.
///
...
struct Brief
{
    ...
    /// приложения (pipeline/rank_toplists.py) и приезжает готовым: приводить к одной
    /// шкале списки разной длины, семейства сайтов и треды разного размера —
    /// работа сборки данных, а не консоли.
    int score      = 0;
    int year       = 0;
    /// -1 — размер неизвестен; карточка в этом случае показывает прочерк
    /// вместо «0 МБ». Отбор и порядок это поле больше не использует.
    long long romSize = -1;
