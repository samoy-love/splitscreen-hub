...
/// Доводит до конца подмену, прерванную между двумя rename (выключили
/// консоль): если нас запустили из резервной копии <имя>.old.nro, а основного
/// <имя>.nro нет, копия возвращает себе основное имя и просит hbloader
/// перезапустить приложение уже оттуда. Звать в самом начале main, до
/// hasPending(). true — файл перемещён, из main нужно сразу выйти.
bool recoverInterruptedSwap();

/// Убирает хвосты прошлых обновлений: <имя>.old.nro после удачного старта
/// новой версии (и <self>.old от прежних сборок) и <self>.new без метки —
/// обрыв или сборка, которая ещё не умела подменять себя при перезапуске.
