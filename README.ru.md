...
старте. Файл без совпавшей суммы не ставится. На время подмены прежняя сборка
лежит рядом как `SplitScreenHub.old.nro`: если консоль выключится посреди
подмены, в Homebrew Menu останется эта копия, и запущенная она сама вернёт
себе обычное имя и доведёт обновление до конца. Код —
[`updater.cpp`](app/source/updater.cpp).
...
- `details.bin` (~5 МБ) — тексты и ссылки карточки, по записи на игру, читаются
  по смещению. Записи сжаты zlib с общим словарём в 32 КБ: поодиночке они
